...
layout = [[sg.TabGroup([[
    sg.Tab('info',info_tab),sg.Tab('donwload',download_tab)
]])]]

window = sg.Window('NuTube',start_layout)
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        break
    if event == 'Submit':
        video_object = YouTube(value['-INPUT-'],on_progress_callback = progress_check, on_complete_callback = on_complete)
        window.close()

        #video info
        window = sg.Window('NuTube', layout,finalize = True)
        ...
        window['-DESCRIPTON-'].update(video_object.description)
